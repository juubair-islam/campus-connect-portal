...
// - NOT enrolled by current user (no enrollment record)
...
<style>
  ...
  .modal {
    ...
    padding: 1.5em;
    border-radius: 8px;
    box-shadow: 0 0 15px rgba(0,124,199,0.3);
  }
  .modal h3 {
    margin-top: 0;
    color: #007cc7;
  ...
  #messageBox {
    ...
    padding: 1em 1.5em;
    border-radius: 6px;
    box-shadow: 0 0 10px rgba(0,124,199,0.7);
    z-index: 11000;
    font-weight: 600;
  }
  #messageBox.error {
...
<nav class="top-nav">
  ...
  <!-- Learner Dropdown -->
  <div class="dropdown">
    <a href="#" class="dropbtn" class="active">Learner▾</a>
    <div class="dropdown-content">
        ...
        <tr data-course='<?php echo json_encode($course, JSON_HEX_APOS|JSON_HEX_QUOT); ?>'>
          ...
          <td>
            <?php
              ...
              } elseif ($course['request_status'] === 'accepted') {
                echo '<span style="color:green;font-weight:600;">Accepted</span>';
              } elseif ($course['request_status'] === 'rejected') {
          ...
          <td>
            <?php if ($course['request_status'] === 'pending'): ?>
              <button class="action-btn cancel-request-btn" data-course-id="<?php echo $course['course_id']; ?>">Cancel Request</button>
            <?php elseif ($course['request_status'] === 'accepted'): ?>
              <em>Enrolled</em>
            <?php else: ?>
              <button class="action-btn send-request-btn" data-course-id="<?php echo $course['course_id']; ?>">Send Request</button>
            <?php endif; ?>
          </td>
        </tr>
      <?php endforeach; ?>
...
<script>
  ...
  // Send Request button click: open send modal with course details
  ...
  sendConfirmBtn.addEventListener('click', () => {
    ...
    .then(data => {
      ...
      if (data.success) {
        // Update button to pending and disable
  ...
  // Cancel Request button click: open cancel modal
